...
- [x] Add field-level comparison logic that records exactly which key fields are present in CHM and absent in Markdown, and exactly which fields are present in Markdown but not in CHM.
  - Extended `PageAuditRecord` with explicit `missing_in_markdown_fields` and `missing_in_html_fields` lists, populated directly from the existing blocking-field comparison pass so the per-field booleans and aggregate deltas cannot drift.
  - Surfaced the new field-level delta lists in both the Markdown report and JSON payload, and expanded `tests/test_audit_chm_detail_parity.py` to assert the new outputs for clean and mismatched parity cases.
  - Verified with `python -m pytest tests/test_audit_chm_detail_parity.py tests/test_compare_chm_to_docs.py -q` -> `25 passed in 1.08s`.
- [x] Expand `tests/test_audit_chm_detail_parity.py` with representative HTML and Markdown fixtures that cover type pages, member pages, overload pages, enum pages, and archive exceptions, and assert the exact extracted counts and field-presence maps.
  - Added `test_representative_fixtures_assert_exact_counts_and_field_presence_maps()` plus shared `presence_map()` and `fixture_metrics()` helpers so the suite now locks exact extractor outputs for all required page shapes, including archive exception field deltas.
  - Verified with `python -m pytest tests/test_audit_chm_detail_parity.py tests/test_compare_chm_to_docs.py -q` -> `26 passed in 1.12s`.
- [x] Add at least one regression test proving that a title-only page with high textual overlap but missing code signatures still fails as `blocking`, so the deeper audit cannot be satisfied by superficial text similarity.
  - Verified existing regression coverage in `tests/test_audit_chm_detail_parity.py::test_title_only_markdown_with_high_textual_overlap_still_blocks_without_signatures`, which asserts `density_ratio > 0.8` while still requiring `severity == "blocking"` when both signature fields are missing from Markdown.
  - Re-ran the focused regression with `python -m pytest tests/test_audit_chm_detail_parity.py -k title_only_markdown_with_high_textual_overlap_still_blocks_without_signatures -q` -> `1 passed, 17 deselected in 0.02s`.
- [x] Run `python -m pytest tests/test_audit_chm_detail_parity.py tests/test_compare_chm_to_docs.py -q` and record passing output in the task comment before marking the phase complete.
  - Verified with `python -m pytest tests/test_audit_chm_detail_parity.py tests/test_compare_chm_to_docs.py -q` -> `26 passed in 1.15s`.
