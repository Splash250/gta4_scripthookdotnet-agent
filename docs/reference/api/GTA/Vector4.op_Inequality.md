# Inequality Operator

Tests for inequality between two objects.

## Visual Basic

Public Shared Operator <>(left As [GTA.Vector4](Vector4.md), right As [GTA.Vector4](Vector4.md)) As [Boolean](http://msdn.microsoft.com/library/default.asp?url=/library/en-us/cpref/html/frlrfSystemBooleanClassTopic.asp)

## C#

public static [bool](http://msdn.microsoft.com/library/default.asp?url=/library/en-us/cpref/html/frlrfSystemBooleanClassTopic.asp) operator !=(
   [GTA.Vector4](Vector4.md) *left*,
   [GTA.Vector4](Vector4.md) *right*
);

#### Parameters

*left*
:   The first value to compare.

*right*
:   The second value to compare.

#### Return Value

`true` if *left* has a different value than *right*; otherwise, `false`.

#### See Also

[Vector4 Class](Vector4.md) | [GTA Namespace](index.md)
